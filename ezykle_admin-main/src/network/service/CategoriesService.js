import { SECTIONS_API_PATH } from "../config/apiPaths";
...
export const listRequest = (data) => async () => {
  try {
    let result = await getRequest(
      `${SECTIONS_API_PATH.LIST_PATH}?limit=${data.limit}&offset=${data.offset}`,
      null,
      false
    );
    return result;
...
export const updateRequest = (data) => async () => {
  try {
    let result = await postRequest(
      SECTIONS_API_PATH.CREATE_UPDATE,
      data,
      false
    );
    return result;
...
export const sectionDataRequest = () => async () => {
  try {
    let result = await getRequest(
      `${SECTIONS_API_PATH.SECTION_DATA}`,
      null,
      false
    );
    return result;
...
export const userSectionDataRequest = (data) => async () => {
  try {
    let result = await getRequest(
      `${SECTIONS_API_PATH.SECTION_DATA}?userId=${data.userId}`,
      null,
      false
    );
    return result;
